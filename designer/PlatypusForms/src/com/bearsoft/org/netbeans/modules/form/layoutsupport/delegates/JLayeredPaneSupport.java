...
public class JLayeredPaneSupport extends AbsoluteLayoutSupport {
    ...
    public void addComponentsToContainer(Container container,
        ...
        for (int i = 0; i < components.length; i++) {
            ...
            if (constraints instanceof LayeredLayoutConstraints) {
                Component comp = components[i];
                container.add(comp, constraints.getConstraintsObject(), i + index);

                Rectangle bounds = ((LayeredLayoutConstraints) constraints).getBounds();
     ...
     * CodeStatement[] statements = CodeStructure.filterStatements( it,
     * getSetBoundsMethod()); if (statements.length > 0) { CodeStatement
     * boundsStatement = statements[statements.length-1];
     * constr.readPropertyExpressions( boundsStatement.getStatementParameters(),
     * 1); constrCode.addStatement(boundsStatement); }
     *
     * FormCodeSupport.readPropertyExpression(constrExp,
     * constr.getProperties()[0], false);
     *
     * return constr; }
     */
    /**
     ...
     * { if (!(constr instanceof LayeredConstraints)) return null;
     *
     ...
     *
     * CodeStatement boundsStatement = CodeStructure.createStatement( compExp,
     * getSetBoundsMethod(),
    ...
    protected LayoutConstraints<?> createDefaultConstraints() {
        return new LayeredLayoutConstraints(0, 0, 0, -1, -1);
    }
    ...
    protected LayoutConstraints<?> createNewConstraints(
        ...
        int layer = currentConstr instanceof LayeredLayoutConstraints
                ? ((LayeredLayoutConstraints) currentConstr).getLayer() : 0;

        return new LayeredLayoutConstraints(layer, x, y, w, h);
    }
     ...
     * property.
     */
    public static class LayeredLayoutConstraints extends AbstractAbsoluteLayoutConstraints implements LayoutConstraints<Integer> {

        protected int layer;
        
        public LayeredLayoutConstraints(int aLayer, int x, int y, int w, int h) {
            super(x, y, w, h);
            layer = aLayer;
            nullMode = true;
        }
        ...
        // ------
        @Override
        public Integer getConstraintsObject() {
            return new Integer(layer);
        }

        @Override
        public LayeredLayoutConstraints cloneConstraints() {
            return new LayeredLayoutConstraints(layer, x, y, w, h);
        }
        ...
        protected FormProperty<?>[] createProperties() {
            FormProperty<?>[] props = super.createProperties();
            FormProperty<?>[] layeredProps = new FormProperty<?>[props.length + 1];

            layeredProps[0] =
                    new FormProperty<Integer>("LayeredConstraints layer", // NOI18N
                    Integer.TYPE,
