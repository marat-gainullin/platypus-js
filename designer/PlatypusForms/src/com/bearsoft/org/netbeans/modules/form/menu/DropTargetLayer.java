...
import com.bearsoft.org.netbeans.modules.form.menu.DropTargetLayer.DropTargetType;
import com.bearsoft.org.netbeans.modules.form.menu.MenuEditLayer.SelectedPortion;
...
/**
 * This component handles drawing all drop targets for the menu bar and the
 * selection rects around selected menu item.
 *
 * Also note that this class attempts to calculate the position of the icon and
 * accelerator within the menu item. It cannot do this completely accurately
 * since the API for JMenuItem does not expose such metrics. This class
 * therefore contains some hard coded values and attempts to use L&F specific
 * information to fine tune the accelerator positioning.
 *
 * @author joshy
 */
class DropTargetLayer extends JComponent {
    ...
    public void setDropTarget(RADComponent<?> comp, Point pt) {
        currentTargetComponent = canvas.formDesigner.getComponent(comp);
        currentTargetPoint = pt;
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (currentTargetComponent != null) {
            if (canvas.formDesigner != null) {
                g2.setColor(DROP_TARGET_COLOR);
                g2.setStroke(DROP_TARGET_LINE_STROKE);

                // if over the menubar itself, rather than a toplevel JMenu
                if (currentTargetComponent instanceof JMenuBar){//(canvas.formDesigner.getRadComponent(currentTargetComponent) == canvas.getFormMenuBar()) {
    ...
    private void drawSelectedComponent(Graphics2D g2, Component selected, RADComponent<?> rad) {
        if (selected != null) {
