...
/**
 * Main interface for working with various layouts of visual containers in Form
 * Editor. An implementations of this interface should hold some metadata
 * representing the layout, be able to set up live components and containers
 * (according to the metadata), handle code generation and persistence, provide
 * drag&drop and resizing support, etc. This interface is very general, it is
 * not recommended to implement it directly. There is a default implementation -
 * AbstractLayoutSupport - which provides most of the necessary functionality,
 * assuming that the supported layout works with certain patterns and rules.
 *
...
public interface LayoutSupportDelegate {
     ...
     * three types of initialization which must be supported: (1) default
     * initialization for an empty (newly created) layout (lmInstance == null,
     * fromCode == false), (2) initialization from an already existing instance
     * of LayoutManager (lmInstance != null, fromCode == false), (3)
     * initialization from persistent code structure, (lmInstance == null,
     * fromCode == true).
     *
    ...
    /**
     * Gets the supported layout manager or container class. Container class is
     * returned if the delegate is "dedicated" to some special container rather
     * than to a layout manager used generally for any container.
     *
    ...
    /**
     * States whether this delegate class is dedicted to some special container
     * layout.
     *
     * @return true if the delegates supports just certain container only, false
     * if the delegates supports a layout manager for use in any container
     ...
     * @return icon to be displayed for node in Component Inspector, null if no
     * icon is provided
     */
     ...
     * editable by the user in Component Inspector when layout node is selected.
     * These are not properties of individual component constraints (see
     * LayoutConstraints.getProperties() for that).
     *
     ...
     * instance (using Customizer.setObject(...) method). Note: If the layout
     * delegate provides special customizer from getSupportCustomizer() method,
     * it should still return its class here so it is apparent that there is
     * some customizer provided.
     *
     ...
     * instance). Note: If the layout delegate provides the customizer here, the
     * class of the customizer should be also returned from getCustomizerClass()
     * method.
     *
     ...
     * error or warning message, the exception is not reported outside. To
     * accept any components simply do nothing here.
     *
     ...
     */
    void acceptNewComponents(RADComponent<?>[] comps,
     ...
     * the exception is not reported outside. To accept any changes simply do
     * nothing here.
     *
     ...
     * changed by the user. The delegate implementation may check if the layout
     * is valid after the change and throw PropertyVetoException if the change
    ...
    /**
     * Adds new components to the layout. (This is intended just at the metadata
     * level, no real components are added in fact.)
     *
     * @param compExpressions array of CodeExpression objects representing the
     ...
     */
    void addComponents(RADComponent<?>[] comps,
            LayoutConstraints<?>[] constraints,
            int index);
    ...
    /**
     * Sets layout constraints for a component at the given index.
     *
     * @param index index of the component in the layout
     * @param aConstraints Constraints to setted.
     */
    void setConstraints(int index, LayoutConstraints<?> aConstraints);
    /**
     ...
     * JTabbedPane it might switch the selected TAB.
     *
     ...
     * should store it and use it e.g. in arrangeContainer method.
     *
