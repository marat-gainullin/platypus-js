...
public class NullLayoutSupport extends AbsoluteLayoutSupport {
     ...
     */
    private static String iconURL = "com/bearsoft/org/netbeans/modules/form/layoutsupport/resources/NullLayout.gif"; // NOI18N
    /**
     * The icon for NullLayout.
     */
    private static String icon32URL = "com/bearsoft/org/netbeans/modules/form/layoutsupport/resources/NullLayout32.gif"; // NOI18N
     ...
     * @Override public CodeGroup getComponentCode(int index) { // hack: be sure
     * that the constraints object is associated with the // primary component
     * (to be able to get its preferred size) LayoutConstraints constr =
     * getConstraints(index); if (constr instanceof AbsoluteLayoutConstraints) {
     * AbsoluteLayoutConstraints absConstr = (AbsoluteLayoutConstraints) constr;
     * if (absConstr.refComponent == null) { absConstr.refComponent =
     ...
     * code
     * @return created CodeExpression representing the layout manager (so
     * representing null value in this case)
     * @Override protected CodeExpression createInitLayoutCode(CodeGroup
     * layoutCode) { return
     * getCodeStructure().createNullExpression(LayoutManager.class); }
     */
    /**
     ...
     * layout constraints information is read. The special thing for null layout
     * is that components are initailized with setBounds call instead of using
     * constraints object, so we must override the reading process from
     ...
     * (getSimpleAddMethod().equals(statement.getMetaObject())) { CodeExpression
     ...
     * -1, -1); constr.nullMode = true; // constr.refComponent =
     * getLayoutContext().getPrimaryComponent(index);
     *
     * // search for setBounds statement on component Iterator it =
     * CodeStructure.getDefinedStatementsIterator(compExp); CodeStatement[]
     * statements = CodeStructure.filterStatements( it, getSetBoundsMethod());
     * if (statements.length > 0) { CodeStatement boundsStatement =
     * statements[statements.length - 1]; constr.readPropertyExpressions(
     * boundsStatement.getStatementParameters(), 0);
     * componentCode.addStatement(boundsStatement); }
     * getConstraintsList().add(constr);
     *
     * return compExp; } return null; }
     */
    /**
     * Creates code for a component added to the layout (opposite to
     * readComponentCode method). As well as for readComponentCode - null layout
     * requires the components to be initailized with setBounds call instead of
     * using constraints object, so this method must be overridden (from
     * AbstractLayoutSupport).
     *
     ...
     * CodeExpression compExp, int index) { // create code for "add" method
     * componentCode.addStatement( CodeStructure.createStatement(
     * getActiveContainerCodeExpression(), getSimpleAddMethod(), new
     * CodeExpression[]{compExp}));
     *
     * // create code for "setBounds" method LayoutConstraints constr =
     * getConstraints(index); if (constr instanceof AbsoluteLayoutConstraints) {
     * AbsoluteLayoutConstraints absConstr = (AbsoluteLayoutConstraints) constr;
     * absConstr.nullMode = true; absConstr.refComponent =
     * getLayoutContext().getPrimaryComponent(index);
     *
     * componentCode.addStatement( CodeStructure.createStatement( compExp,
     * getSetBoundsMethod(),
     * absConstr.createPropertyExpressions(getCodeStructure(), 0))); } }
     *
     * private static Method getSetBoundsMethod() { if (setBoundsMethod == null)
     * { try { setBoundsMethod = Component.class.getMethod( "setBounds", //
     * NOI18N new Class<?>[]{Integer.TYPE, Integer.TYPE, Integer.TYPE,
     * Integer.TYPE}); } catch (NoSuchMethodException ex) { // should not happen
     * ErrorManager.getDefault().notify(ex); } } return setBoundsMethod; }
     */
}
