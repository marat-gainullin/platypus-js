...
/**
 * A layer used by FormDesigner for in-place editing of components' labels and
 * text. Can be used in two modes - layer editing or direct editing. In layer
 * editing mode, a JTextField-based component is set up on the layer for editing
 * labels and buttons. In direct editing mode, the edited component is used also
 * for editing, the layer just ensures that other components are inaccessible.
 *
 * @author Tomas Pavek
 */
...
//                               +", showing: "+editingTextComp.isShowing());
    ...
    // -----------
    /**
     * Custom JTextField used as editing component on the layer. It prevents
     * focus manager from switching to another components (using TAB and
     * Shift+TAB). It also changes its size according to entered text, with
     * respect to underlying (edited) component's size.
     */
            ...
            if (prefWidth != bounds.width) {
                ...
                if (prefWidth == baseBounds.width) {
                    ...
                    if (hA == SwingConstants.LEFT
                        ...
                        if (cX - leftX < dX1 + over1) {
                            dX1 = cX - leftX;
                        } else {
                            dX1 += over1;
                        }
                        if (rightX - cX < dX2 + over2) {
                            dX2 = rightX - cX;
                        } else {
                            dX2 += over2;
                        }
