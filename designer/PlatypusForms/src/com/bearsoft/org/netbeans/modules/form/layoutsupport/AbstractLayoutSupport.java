 ...
 *
 * This class provides basic support for layouts following these rules: (1) the
 * supported layout manager is a JavaBean (it means that it has an empty public
 * constructor, the parameters are set as properties, BeanInfo is used to obtain
 * the properties), (2) Container.setLayout(LayoutManager) method is used on the
 * container to set the layout, (3) Container.add(Component) and
 * Container.add(Component, Object) methods are used on the container to add
 * components (the second method for adding with layout constraints).
 *
...
public abstract class AbstractLayoutSupport implements LayoutSupportDelegate {
    ...
    private RADLayout radLayout;
    private FormProperty<?>[] allProperties;

    // ------------------
    // LayoutSupportDelegate interface implementation
    /**
     * Initialization of the layout delegate before the first use. There are
     * three types of initialization supported: (1) default initialization for
     * an empty (newly created) layout (lmInstance == null, fromCode == false),
     * (2) initialization from an already existing instance of LayoutManager
     * (lmInstance != null, fromCode == false), (3) initialization from
     * persistent code structure, (lmInstance == null, fromCode == true).
     *
    ...
    public void initialize(LayoutSupportContext aLayoutContext,
        ...
        if (cls != null && LayoutManager.class.isAssignableFrom(cls)) {
            ...
            if (lmInstance != null) {
                radLayout = new RADLayout(this, lmInstance);
                deriveChangedPropertiesFromInstance(radLayout);
            }
        } else {
            radLayout = null;
        }
     ...
     *
     * @return true if only certain container is supported, false if a layout
     * manager for use in any container is supported
     */
    @Override
    ...
    /**
     * For dedicated supports: check whether given default container instance is
     * empty. Default implementation returns true - it's up to subcalsses to
     * check the special containers.
     *
     ...
     * Only 16x16 color icon is required. The default implementation tries to
     * obtain the icon from BeanInfo of the layout manager.
     *
    ...
    public Image getIcon(int type) {
        if (radLayout != null) {
            Image icon = radLayout.getBeanInfo().getIcon(type);
            if (icon != null) {
                return icon;
            }
        }
    ...
    /**
     * This method provides properties of the supported layout - if it is a
     * JavaBean class implementing LayoutManager. The properties are obtained
     * from the BeanInfo of the layout manager. Note these are not properties of
     * individual components constraints.
     *
    ...
    public Node.PropertySet[] getPropertySets() {
        ...
        if (properties == null) {
            propertySets = radLayout != null
                    ? radLayout.getProperties() : null;
    ...
    /**
     * Returns a class of a customizer for the layout manager being used as a
     * JavaBean. The class should be a java.awt.Component and
    ...
    public Class<?> getCustomizerClass() {
        return radLayout == null ? null
                : radLayout.getBeanInfo().getBeanDescriptor().getCustomizerClass();
    }
     ...
     * vice versa, the customizer can have full control over the layout delegate
     * (unlike the bean customizer which operates only with layout manager bean
     * instance). The default implementation returns null.
     *
    ...
    public int getComponentCount() {
        return componentConstraints != null
                ? componentConstraints.size() : 0;
    }

    /**
     * This method is called to accept new components before they are added to
     * the layout (by calling addComponents method). It may adjust the
     ...
     * error or warning message, the exception is not reported outside. The
     * default implementation accepts any components - simply does nothing.
     *
     ...
     */
    @Override
    public void acceptNewComponents(RADComponent<?>[] comps,
     ...
     * user. Subclasses may check whether the layout is valid after the change
     * and throw PropertyVetoException if the change should be reverted. It's up
     * to the delagate to display an error or warning message, the exception is
     * not reported outside. The default implementation accepts any change.
     *
    ...
    /**
     * This method is called after a constraint property of some component is
     * changed by the user. Subclasses may check if the layout is valid after
     * the change and throw PropertyVetoException if the change should be
     * reverted. It's up to the delagate to display an error or warning message,
     * the exception is not reported outside. The default implementation accepts
     * any change.
     *
     ...
     */
    @Override
    public void addComponents(RADComponent<?>[] newComps,
            LayoutConstraints<?>[] newConstraints,
            int index) {
        if (index < 0 || index > componentConstraints.size()) {
            index = componentConstraints.size();
        }
    ...
    /**
     * Removes all components from the layout (at metadata level). The code
     * structures describing the layout is updated immediately.
     */
    @Override
    ...
    /**
     * Indicates whether there's some change in the layout in comparison with
     * the default layout of given container. If there's no change, no code
     * needs to be delegate (e.g. default FlowLayout in JPanel). Note this is
     * related to the container layout only, not to components.
     *
     ...
     */
    @Override
    public void setConstraints(int index, LayoutConstraints<?> aConstraints) {
        if (index >= 0 && index < componentConstraints.size()) {
            componentConstraints.set(index, aConstraints);
        }
    }
     ...
     * The layout delegate may do something with the container, e.g. for
     * JTabbedPane it might switch the selected TAB. The default implementation
     * does nothing.
     *
     ...
     * Inspector. If the layout delegate is interested in such information, it
     * may store it and use it e.g. in arrangeContainer method. The default
     ...
     * through layout properties and added components. For example, the selected
     * component index can be applied here (see delegates for CardLayout and
     * JTabbedPane). The default implementation does nothing.
     *
     ...
     * component in the container; may return null if the layout does not use
     * component constraints, or if default constraints should be used
     */
    @Override
     ...
     * container; may return -1 if the layout rather uses component constraints,
     * or if a default index should be used
     */
    @Override
    ...
    /**
     * Returns context parameters for the assistant (context sensitive help
     * bar).
     *
    ...
    /**
     * This method should paint a feedback for a component dragged over a
     * container (or just for mouse cursor being moved over container, without
     * any component). In principle, it should present given component layout
     * constraints or index graphically.
     *
     * @param container instance of a real container over/in which the component
     * is dragged
     * @param containerDelegate effective container delegate of the container
     ...
     * @param newIndex component's index position to be presented (if
     * newConstraints == null)
     * @param g Graphics object for painting (with color and line style set)
     * @return whether any feedback was painted (may return false if the
     * constraints or index are invalid, or if the painting is not implemented)
     */
    @Override
    ...
    /**
     * Creates a default instance of LayoutManager (for internal use). Override
     * this method if the layout manager is not a bean (cannot be created from
     * default constructor).
     *
    ...
    protected LayoutManager createDefaultLayoutInstance()
            throws Exception {
        return CreationFactory.<LayoutManager>createDefaultInstance(getSupportedClass());
    }

    /**
     * Cloning method - creates a clone of the reference LayoutManager instance
     * (for external use). Override this method if the layout manager is not a
     * bean (cannot be created from default constructor, and copied using
     * properties).
     *
    ...
    protected LayoutManager cloneLayoutInstance(Container container,
            ...
            throws Exception {
        return radLayout == null ? null
                : radLayout.cloneBeanInstance(null);
    }

    /**
     * Cloning method - creates a new instance of this layout support, just not
     * initialized yet.
     *
    ...
    protected void clean() {
        if (componentConstraints != null) {
            ...
            componentConstraints = new ArrayList<>();
        }
        radLayout = null;
        allProperties = null;
    }
    ...
    protected FormProperty<?> getProperty(String propName) {
        return radLayout == null ? null
                : radLayout.<FormProperty<?>>getProperty(propName);
    }

    /**
     * This method can be overridden to provide other layout properties than the
     * standard ones of LayoutManager handled automatically as a bean. This
     * method is called from getPropertySets() implementation to obtain the
     * default property set for the layout (assuming there's only one property
     * set). So it is also possible to override (more generally)
    ...
    /**
     * Gets the main CodeStructure object (holding all code structure data) for
     * creating/reading the code. Obtained from the layout context.
     *
     * @return main CodeStructure object (for working with code structure data)
     * protected final CodeStructure getCodeStructure() { return
     * layoutContext.getCodeStructure(); }
     */
    /**
     * Gets the internal list of layout constraints of components in the layout.
     * (The list contains instances of LayoutConstraints).
     *
    ...
    /**
     * This method should be used by subclasses if they need to re-create the
     * reference layout manager instance (see BoxLayoutSupport for example).
     */
    protected final void updateLayoutInstance() {
        ...
        if (lm != null && radLayout != null) {
            radLayout.updateInstance(lm);
        }
