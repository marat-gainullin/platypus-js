...
/**
 * ToolBar in the PlatypusFormLayoutView - by default it holds buttons for
 * selection and connection mode and for testing the form. May contain other
 * buttons for some form editor actions.
 *
 * @author Tomas Pavek
 */
class FormToolBar extends ToolbarWithOverflow {
    ...
    void installAlignActions() {
        Collection<Action> actions = formDesigner.getAlignActions();
    ...
    void installResizabilityActions() {
        Collection<Action> actions = formDesigner.getResizabilityActions();
    ...
    void updateDesignerMode(int mode) {
        selectionButton.setSelected(mode == PlatypusFormLayoutView.MODE_SELECT);
        //connectionButton.setSelected(mode == PlatypusFormLayoutView.MODE_CONNECT);
        paletteButton.setSelected(mode == PlatypusFormLayoutView.MODE_ADD);

        if (addLabel.isVisible()) {
            PaletteItem item = PaletteUtils.getSelectedItem();
            if (item != null && mode == PlatypusFormLayoutView.MODE_ADD) {
                addLabel.setIcon(
    ...
    private class Listener extends MouseAdapter
        ...
        public boolean acceptNodes(Node[] nodes) {
            ...
            PaletteItem item = nodes[0].getLookup().lookup(PaletteItem.class);
        ...
        public void mouseReleased(MouseEvent e) {
            if (SwingUtilities.isRightMouseButton(e)
                    && formDesigner.getDesignerMode() == PlatypusFormLayoutView.MODE_SELECT) {
