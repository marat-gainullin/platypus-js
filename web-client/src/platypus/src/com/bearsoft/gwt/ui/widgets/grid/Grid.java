...
public class Grid<T> extends SimplePanel implements ProvidesResize, RequiresResize {
    ...
    public Grid(ProvidesKey<T> aKeyProvider, SpanningSectionsBuilder aLeftHeaderGroupsBuilder, SpanningSectionsBuilder aRightHeaderGroupsBuilder) {
        ...
        //scrollableRightContainer.getElement().getStyle().setOverflow(Style.Overflow.AUTO); default value
        //  context
                    ...
                    if (source.isMove()) {
                        ...
                        if (target != null) {
                            ...
                            if (isForeignColumn || sourceIndex != targetIndex) {
                                ...
                                sourceTable.removeColumn(sourceIndex);// ColumnsRemover will take care about any column sharing.
                                targetSection.insertColumn(targetIndex, column, targetSection == headerLeft || targetSection == headerRight ? header : null, targetSection == footerLeft || targetSection == footerRight ? footer : null);
                                for (AbstractCellTable<T> partner : targetSection.getColumnsPartners()) {
                                    partner.insertColumn(targetIndex, column, partner == headerLeft || partner == headerRight ? header : null, partner == footerLeft || partner == footerRight ? footer : null);
                                }
                                frozenColumns = headerLeft.getColumnCount();
                                targetSection.setColumnWidth(column, width);// ColumnWidthPropagator will take care about any column sharing.
                        ...
                        // Source and target tables atre the same, so we can cast to DraggedColumn<T> with no care
    ...
    private void propagateHeightButScrollable() {
        ...
        // row height is free, but cells' height is setted by hand in order to support gecko browsers 
        //scrollableLeftContainer.getElement().getParentElement().getStyle().setHeight(100, Style.Unit.PCT);
        //scrollableRightContainer.getElement().getParentElement().getStyle().setHeight(100, Style.Unit.PCT);
        // some code for opera...
    ...
    protected void refreshColumns() {
        ...
        for (int i = headerRight.getColumnCount() - 1; i >= 0; i--) {
            ...
            headerRight.removeColumn(i);// ColumnsRemover will take care about columns sharing
        }
        for (int i = headerLeft.getColumnCount() - 1; i >= 0; i--) {
            ...
            headerLeft.removeColumn(i);// ColumnsRemover will take care about columns sharing
        }
    ...
    public int getDataColumnCount() {
        return headerLeft.getColumnCount()
                + headerRight.getColumnCount();
    }
