...
public class Filter extends HashOrderer {
	...
	public void filterRowset(Object... values) throws RowsetException {
		if (values != null && values.length > 0) {
			...
			throw new RowsetException(
					"bad filtering conditions. Absent or empty");
		}
	}
	...
	public void filterRowset(KeySet values) throws RowsetException {
		if (rowset != null) {
			...
			if (!filtering) {
				...
				try {
					if (values != null) {
						if (rowset.getRowsetChangeSupport()
								.fireWillFilterEvent()) {
							if (!caseSensitive) {
							...
							boolean foreignFilter = activeFilter == null
									|| activeFilter != this;
							...
							if (!filterApplied) {
								...
								Set<RowsetListener> l = rowset
										.getRowsetChangeSupport()
										.getRowsetListeners();
								rowset.getRowsetChangeSupport()
										.setRowsetListeners(null);
								try {
									if (wasBeforeFirst) {
										rowset.beforeFirst();
						...
						if (originalRows != null) {
							if (rowset.getRowsetChangeSupport()
								...
								rowset.getRowsetChangeSupport()
										.setRowsetListeners(null);
								try {
									boolean positioned = rowset
											.absolute(originalPos);
	 ...
	 * rowset itself, such as refresh of the whole data.
	 */
	 ...
	 */
	@Override
	public void build() throws RowsetException { // check to avoid filter on
													// filter
		if (!constrainting) {
	...
	public void apply(JavaScriptObject aValues) throws Exception {
		...
		if (getFields().size() == fieldsValues.length()) {
			...
			throw new IllegalArgumentException(
					"Filtering keys must correspond to filtering fields were specified while filter creation (may be createFiltering call)");
	}
}
