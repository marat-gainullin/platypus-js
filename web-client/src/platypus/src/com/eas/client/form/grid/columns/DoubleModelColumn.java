...
public class DoubleModelColumn extends ModelColumn<Double> {

	public DoubleModelColumn(String aName) {
		super(new TreeExpandableCell<Row, Double>(new DoubleEditorCell()), aName, null, new DoubleRowValueConverter());
		setEditor(new ModelSpin());
		((DoubleEditorCell) getTargetCell()).setRenderer(new CellRenderer<Double>() {
			@Override
			public boolean render(Context context, String aId, Double value, SafeHtmlBuilder sb) {
				DoubleModelColumn column = DoubleModelColumn.this;
				...
				if (onRender != null) {
					try {
						...
						if (cellToRender != null) {
							...
							DoubleModelColumn.this.bindGridDisplayCallback(decorId, cellToRender);
	...
	@Override
	public void setEditor(ModelDecoratorBox<Double> aEditor) {
