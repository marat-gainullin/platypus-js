...
public class ListStoreFiller extends RowsetAdapter implements PropertyChangeListener {
	...
	public ListStoreFiller(ListStore<Row> aStore, Entity aRowsetHost, Set<Entity> toEnsure) {
		super();
		store = aStore;
		...
		if ("rowset".equals(evt.getPropertyName())) {
			if (evt.getOldValue() == null && evt.getNewValue() != null) {
				...
				Scheduler.get().scheduleDeferred(new ScheduledCommand() {
					@Override
					public void execute() {
						((Entity) evt.getSource()).getChangeSupport().removePropertyChangeListener(ListStoreFiller.this);
					}
				});
				if (--deferred == 0) {
					rowsetError = null;
