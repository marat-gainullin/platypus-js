...
public class Toolbar extends Box {
    ...
    protected Element altContent = Document.get().createDivElement();
    protected Element chevron = Document.get().createDivElement();

    public Toolbar() {
        super();
        getElement().addClassName("toolbar");
        chevron.addClassName("toolbar-chevron");
        chevron.getStyle().setDisplay(Style.Display.NONE);
        chevron.getStyle().setPosition(Style.Position.ABSOLUTE);
        chevron.getStyle().setHeight(100, Style.Unit.PCT);

        element.addClassName("toolbar-content");
        element.addClassName("btn-group");
        element.appendChild(chevron);
        chevron.<XElement>cast().addEventListener(BrowserEvents.CLICK, new XElement.NativeHandler() {
            @Override
