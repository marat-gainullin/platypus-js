...
define(['boxing', 'common-utils/color', 'common-utils/cursor', 'common-utils/font', './action-event', './cell-render-event', './component-event', './focus-event', './item-event', './key-event', './value-change-event', './popup-menu'], function(B, Color, Cursor, Font, ActionEvent, RenderEvent, ComponentEvent, FocusEvent, ItemEvent, KeyEvent, ValueChangeEvent, PopupMenu) {
     ...
     */
    function RadioButton(text, selected, actionPerformed) {
        var maxArgs = 3;
        var delegate = arguments.length > maxArgs ?
              arguments[maxArgs] 
            : arguments.length === 3 ? new javaClass(B.boxAsJava(text), B.boxAsJava(selected), B.boxAsJava(actionPerformed))
            : arguments.length === 2 ? new javaClass(B.boxAsJava(text), B.boxAsJava(selected))
    ...
    space.putPublisher(className, function(aDelegate) {
        return new RadioButton(null, null, null, aDelegate);
    });
    return RadioButton;
});