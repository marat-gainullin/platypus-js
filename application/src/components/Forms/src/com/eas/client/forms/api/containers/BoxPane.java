 ...
 * @author mg
 */
public class BoxPane extends JPanel implements HasPublished, HasContainerEvents, HasChildren, HasPublishedInvalidatableCollection {
    ...
    public int getOrientation() {
        int axis = ((BoxLayout) super.getLayout()).getAxis();
    ...
    public int getHgap() {
        return ((BoxLayout) super.getLayout()).getHgap();
    }

    @ScriptFunction
    public void setHgap(int aValue) {
        ((BoxLayout) super.getLayout()).setHgap(aValue);
        super.revalidate();
        super.repaint();
    }
    ...
    public int getVgap() {
        return ((BoxLayout) super.getLayout()).getVgap();
    }

    @ScriptFunction
    public void setVgap(int aValue) {
        ((BoxLayout) super.getLayout()).setVgap(aValue);
        super.revalidate();
        super.repaint();
    }
    ...
    public void add(JComponent aComp) {
        if (aComp != null) {
            aComp.addPropertyChangeListener(resizer);
            super.add(aComp);
            super.revalidate();
            super.repaint();
        }
    ...
    @Override
    public void clear() {
        for (java.awt.Component comp : super.getComponents()) {
    ...
    void ajustSize() {
        if ((super.getParent() instanceof JViewport && super.getParent().getParent() instanceof JScrollPane)
                || (super.getParent() instanceof JComponent && ((JComponent) super.getParent()).getLayout() instanceof BoxLayout)) {
            Dimension newsize = new Dimension(0, 0);
            BoxLayout boxLayout = (BoxLayout) super.getLayout();
            java.awt.Component[] comps = super.getComponents();
            if (comps.length > 0) {
                ...
                if (axis == BoxLayout.LINE_AXIS || axis == BoxLayout.X_AXIS) {
                    newsize.height = super.getParent().getHeight();
                    newsize.width += boxLayout.getHgap() * (comps.length - 1);
                } else {
                    newsize.width = super.getParent().getWidth();
    ...
    protected class Resizer implements ContainerListener, HierarchyBoundsListener, PropertyChangeListener {
        ...
        public void propertyChange(PropertyChangeEvent evt) {
            if ("preferredSize".equals(evt.getPropertyName())) {
                ...
                if (sourceComp.getParent() == BoxPane.this) {
