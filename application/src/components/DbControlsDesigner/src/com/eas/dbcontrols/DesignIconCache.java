 ...
 * @author mg
 */
public class DesignIconCache {

    private static final Map<String, ImageIcon> icons = new HashMap<>();
    private static final String iconsPrefix = DesignIconCache.class.getPackage().getName().replace('.', '/') + "/resources/";

    public static ImageIcon getIcon(String iconName) {
        if (iconName != null) {
            ImageIcon lic = icons.get(iconName);
            if (lic == null) {
                URL url = DesignIconCache.class.getClassLoader().getResource(iconsPrefix + iconName);
                if (url != null) {
