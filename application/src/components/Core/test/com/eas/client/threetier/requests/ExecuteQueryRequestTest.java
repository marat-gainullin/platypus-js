...
public class ExecuteQueryRequestTest extends RequestsBaseTest {
    ...
    public void testWriteGeometryData() throws Exception {
        ...
        Parameters params = new Parameters();
        Parameter param = new Parameter();
        param.setTypeInfo(DataTypeInfo.GEOMETRY);
        param.setValue(null);
        params.add(param);
        param = new Parameter();
        param.setTypeInfo(DataTypeInfo.GEOMETRY);
    ...
    public void testReadGeometryData() throws Exception {
        ...
        DataTypeInfo gTypeInfo = DataTypeInfo.GEOMETRY;
        ByteArrayOutputStream outStream = new ByteArrayOutputStream();
        ProtoWriter writer = new ProtoWriter(outStream);
        writer.put(RequestsTags.TAG_QUERY_ID, queryId);
        writer.put(RequestsTags.TAG_SQL_PARAMETER);
        ...
        assertEquals(DataTypeInfo.GEOMETRY.getSqlType(), instance.getParams().get(2).getTypeInfo().getSqlType());
        assertEquals("JTS_Geometry", instance.getParams().get(2).getTypeInfo().getSqlTypeName());
        assertEquals(Object.class.getName(), instance.getParams().get(2).getTypeInfo().getJavaClassName());
