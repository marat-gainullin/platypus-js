...
begin
  ...
  if majorVersion>=MIN_VISTA_VERSION then Result:=true;
end;
...
begin
  ...
  settingsFile:=TMemIniFile.Create(currentDirectory+'settings.set');
  try
    ...
    CONFIG_NAME:=currentDirectory+settingsFile.ReadString('Update', 'ConfigFileName', 'update.xml');
  ...
  if FileExists(newFile) then
   begin
     if (DeleteFile(oldFile)) then
       if RenameFile(newFile,oldFile) then Result:=true;
   end;
end;
...
begin
  if getNeedUAC then
    runApp(currentDirectory+'platypus.exe', UPDATE_PARAM, HIDE_APPLICATION, true, true)
  else runApp(currentDirectory+'platypus.exe', UPDATE_PARAM, HIDE_APPLICATION, false, true);
end;
...
begin
  settingsFile:=TMemIniFile.Create(currentDirectory+'settings.set');
  try
    Result := settingsFile.ReadString('Update', 'RunFile', '');
  finally
    settingsFile.Destroy;
  end;
end;
...
begin
  ...
  if ParamStr(1)=UPDATE_PARAM then
   begin
     if runApp('javaw', generateRunParams('update'), SHOW_APPLICATION, false, true)<>ERROR_RESULT then
      begin
        replaceUpdaterFile();
        Close;
        Exit;
   ...
   begin
     if runApp('javaw', generateRunParams('newversion'), SHOW_APPLICATION, false, true)=NEED_UPDATE_RESULT then
       beginUpdate;
  ...
  runFile := getRunFile();
  if runFile <> '' then
    runApp('WScript.exe', '"'+currentDirectory+runFile+'"',HIDE_APPLICATION);
  Close();
end;
